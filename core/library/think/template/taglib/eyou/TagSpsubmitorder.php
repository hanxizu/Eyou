...
{ 
    ...
    public function getSpsubmitorder()
    {
        ...
        // 会员模板版本号
        if (in_array($this->usersTplVersion, ['v2', 'v4'])) {
            $ShopAddressInfo = [];
        ...
        if (!empty($PayApiList)) {
            foreach ($PayApiList as $key => $value) {
                ...
                if ('wechat' == $value['pay_mark']) {
                    ...
                    // 支付宝判断
                    if ((isset($PayInfo['is_open_alipay']) && 0 == $PayInfo['is_open_alipay']) || false === $this->findHupijiaoIsExis('alipay')) {
                        $use_pay_type = 1;
