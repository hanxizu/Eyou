...
$(function() {
    ...
    if (addr_id && !jsonData.onlyVerify) SelectEd('addr_id', addr_id);
});
...
function selectLogisticsType(logisticsType) {
    ...
    // 到店自提
        ...
        if (0 < parseInt(store_id)) {
            ...
            if (0 === parseInt(jsonData.is_wap)) $("#selectVerifyID").css('display', '');
        } else {
...
function selectVerifyStore(confirm, obj) {
    if (confirm) {
        ...
        if (0 < parseInt($(obj).data('store_id'))) {
            ...
            if (1 === parseInt(jsonData.is_wap)) $("#selectVerifyID").css('display', 'none');
        } else {
...
function orderFreightCountLogic(freight) {
    ...
    // 计算总价+运费
    var totalAmountOld = $('#TotalAmount_old').val();
    var amountNew = (Number(totalAmountOld) + Number(freight)).toFixed(2);
